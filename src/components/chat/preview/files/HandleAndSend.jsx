...
import { reomveFileFromFiles, sendMessage } from "../../../../features/chatSlice";
...
function HandleAndSend({ activeIndex, setActiveIndex, message, socket }) {
  ...
  const handleRemoveFile = (index) => {
    dispatch(reomveFileFromFiles(index))
  };
  return (
    <div className="w-[97%] flex items-center justify-between mt-2 border-t dark:border-dark_border_2">
      ...
      <div className="flex items-center gap-x-2">
        {
          files.map((file, i)=>(
            <div key={i} className={`fileThumbnail relative w-16 h-16 border dark:border-white mt-2 rounded-md overflow-hidden cursor-pointer 
            ...
            `} onClick={()=>setActiveIndex(i)}>
              ...
              <div className="removeFileIcon hidden"
                ...
                <CloseIcon className="dark:fill-dark_svg_1 absolute right-0 top-0 "/>
              </div>
            </div>
        ))}
        {/* Add Another file */}
